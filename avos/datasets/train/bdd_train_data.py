"""
BDD dataloader with train data augmented by ytvos.
"""
import math
import torch
...
class BddTrainDataset(torch.utils.data.Dataset):
    def __init__(self, num_frames=6, use_ytvos=False, train_size=300, use_flow=False):
        ...
        self.bdd_train_seqs_file = dataset_path_config.bdd_train_seqs_file
        self.bdd_rgb_path = dataset_path_config.bdd_rgb_path
        self.bdd_gt_path = dataset_path_config.bdd_gt_path
        ...
        self.img_ids = []
        logger.debug('loading BDD train seqs...')
        with open(self.bdd_train_seqs_file, 'r') as f:
            video_names = f.readlines()
            video_names = [name.strip() for name in video_names]
            logger.debug('BDD-train num of videos: {}'.format(len(video_names)))
            for video_name in video_names:
                frames = sorted(glob.glob(os.path.join(self.bdd_gt_path, video_name, '*.png')))
    ...
    def __getitem__(self, idx):
        ...
        for frame_id in frame_indices:
            ...
            if dataset == 'ytvos':
                ...
                img_path = os.path.join(self.bdd_rgb_path, video_name, frame_name + '.jpg')
                gt_path = os.path.join(self.bdd_gt_path, video_name, frame_name + '.png')
                if self.use_flow:
