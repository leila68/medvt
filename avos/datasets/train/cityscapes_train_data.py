...
class CityscapesTrainDataset(torch.utils.data.Dataset):
    def __init__(self, num_frames=6, use_ytvos=False, train_size=300, use_flow=False):
        ...
        self.cityscapes_train_seqs_file = dataset_path_config.cityscapes_train_seqs_file
        self.cityscapes_rgb_path = dataset_path_config.cityscapes_rgb_path
        self.cityscapes_gt_path = dataset_path_config.cityscapes_gt_path
        ...
        with open(self.cityscapes_train_seqs_file, 'r') as f:
            video_names = f.readlines()
            video_names = [name.strip() for name in video_names]
            logger.debug('Cityscapes-train num of videos: {}'.format(len(video_names)))
            for video_name in video_names:
                frames = sorted(glob.glob(os.path.join(self.cityscapes_gt_path, video_name, '*.png')))
    ...
    def __getitem__(self, idx):
        ...
        for frame_id in frame_indices:
            ...
            if dataset == 'ytvos':
                ...
                img_path = os.path.join(self.cityscapes_rgb_path, video_name, frame_name + '.png')
                gt_path = os.path.join(self.cityscapes_gt_path, video_name, frame_name + '.png')
                if self.use_flow:
