...
class BddValDataset(torch.utils.data.Dataset):
    def __init__(self, num_frames=6, val_size=473, sequence_names=None, use_flow=False, max_sc=None, style=None):
        ...
        print('BDDValDataset->use_flow:'+str(use_flow))
        ...
        self.bdd_val_seqs_file = dataset_path_config.bdd_val_seqs_file
        self.bdd_rgb_val_path = dataset_path_config.bdd_rgb_val_path
        self.bdd_gt_val_path = dataset_path_config.bdd_gt_val_path
        self.all_styles = dataset_path_config.bdd_styles

        self.bdd_flow_path = dataset_path_config.bdd_flow_path
        self.stylized_bdd_path = dataset_path_config.bdd_stylized_image_path
        ...
        self.img_ids = []
        logger.debug('loading bdd val seqs...')
        if sequence_names is None or len(sequence_names) == 0:
            with open(self.bdd_val_seqs_file, 'r') as f:
                video_names = f.readlines()
                video_names = [name.strip() for name in video_names]
        else:
            video_names = sequence_names
        logger.debug('bdd-val num of videos: {}'.format(len(video_names)))
        for video_name in video_names:
            frames = sorted(glob.glob(os.path.join(self.bdd_gt_val_path, video_name, '*.png')))
            self.frames_info['bdd'][video_name] = [frame_path.split('/')[-1][:-4] for frame_path in frames]
            self.img_ids.extend([('bdd', video_name, frame_index) for frame_index in range(len(frames))])
    ...
    def __getitem__(self, idx):
        ...
        for frame_id in frame_indices:
            ...
            if self.style is None or self.style == 'Original':
                img_path = os.path.join(self.bdd_rgb_val_path, video_name, frame_name + '.jpg')
            else:
                img_path = os.path.join(self.stylized_bdd_path, 'JPEGImages', self.style, video_name,
                                        frame_name + '.jpg')
            # img_path = os.path.join(self.davis16_rgb_path, video_name, frame_name + '.jpg')
            gt_path = os.path.join(self.bdd_gt_val_path, video_name, frame_name + '.png')
            ...
            if self.use_flow and not skip_current_sample_flow:
                ...
                fwd_flow_file = os.path.join(self.bdd_flow_path, video_name,
                                             prev_frame + '_' + frame_name + '.png')
                bkd_flow_file = os.path.join(self.bdd_flow_path, video_name,
