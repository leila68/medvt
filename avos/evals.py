...
from avos.datasets.test.bdd_val_data import BddValDataset as BddValDataset
from avos.datasets.test.cityscapes_val_data import CityscapesValDataset as CityscapesValDataset
...
def compute_gpu_memory(cuda_device=0):
    try:
        nvidia_smi.nvmlInit()
        handle = nvidia_smi.nvmlDeviceGetHandleByIndex(cuda_device)
        info = nvidia_smi.nvmlDeviceGetMemoryInfo(handle)
        nvidia_smi.nvmlShutdown()
        return info.used / 1_000_000  # MB
    except Exception as e:
        print(f"[ERROR] compute_gpu_memory failed: {e}")
        return 0.0
...
def compute_predictions_flip_ms(model, samples, gt_shape, flows=None, ms=True, ms_gather='mean', flip=True,
    ...
    if flip:
        ...
        outputs_flipped = compute_predictions_ms(model, samples_flipped, flows_flipped, gt_shape,
...
@torch.no_grad()
def infer_on_davis_without_gpu_fps(model, data_loader, device, msc=False, flip=False, save_pred=False,
...
def infer_on_davis(model, data_loader, device, msc=False, flip=False, save_pred=False,
    ...
    print(f"[DEBUG] compute_memory = {compute_memory}")
    if compute_memory:
        avg_mem = 0
        peak_gpu_mem = 0
    ...
    for samples, targets in tqdm(data_loader):
        ...
        start = time.time()
        outputs = compute_predictions_flip_ms(model, samples, gt_shape, flows=flows,
                                              ms=msc, ms_gather='mean', flip=flip,
                                              flip_gather='mean', scales=_scales)
        inference_time += time.time() - start

        if compute_memory:
            end_mem = compute_gpu_memory(0)
            avg_mem += end_mem
            if end_mem > peak_gpu_mem:
                peak_gpu_mem = end_mem
        ...
        if save_pred:
            ...
            if not os.path.exists(bm_out_dir):
                os.makedirs(bm_out_dir)
            cv2.imwrite(os.path.join(bm_out_dir, '%s.png' % center_frame_name),
                        (out.astype(np.float32) * 255).astype(np.uint8))
    ...
    print('Avg Inference Time: %.3f sec' % (inference_time))
    print('Avg Inference Time per Frame: %.3f sec' % (inference_time / i_iter))
    ...
    if compute_memory:
        print('Average GPU memory usage is %.2f MB' % (avg_mem / i_iter))
        print('Peak GPU Memory Usage: %.2f MB' % peak_gpu_mem)
...
@torch.no_grad()
def infer_on_kittimots_withour_gpu_fps(model, data_loader, device, msc=False, flip=False, save_pred=False,
...
def infer_on_kittimots(model, data_loader, device, msc=False, flip=False, save_pred=False,
    ...
    logger.debug('Kittimots Videos Mean IOU: %0.3f' % video_mean_iou)
    print('Kittimots Videos Mean IOU: %0.3f' % video_mean_iou)
    return video_mean_iou

@torch.no_grad()
def infer_on_bdd(model, data_loader, device, msc=False, flip=False, save_pred=False,
                   out_dir='./results/bdd/', msc_scales=None):
    if msc and msc_scales is not None:
        _scales = msc_scales
    elif msc and msc_scales is None:
        _scales = [0.75, 0.8, 0.9, 1, 1.1, 1.2, 1.3]
    ...
    with open(os.path.join(out_dir, csv_file_name), 'w') as f:
        ...
        cf.writerow(video_names)
        cf.writerow(video_ious)
    logger.debug('bdd Videos Mean IOU: %0.3f' % video_mean_iou)
    return video_mean_iou


@torch.no_grad()
def infer_on_cityscapes_without_gpu_fps(model, data_loader, device, msc=False, flip=False, save_pred=False,
                       out_dir='./results/cityscapes/', msc_scales=None):
    if msc and msc_scales is not None:
        _scales = msc_scales
    elif msc and msc_scales is None:
        _scales = [0.75, 0.8, 0.9, 1, 1.1, 1.2, 1.3]
    ...
    video_mean_iou = np.mean([np.mean(list(vid_iou_f.values())) for _, vid_iou_f in vid_iou_dict.items()])
    # ### ### Write the results to CSV ### ###
    csv_file_name = 'Cityscapes_results.csv'
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    video_names = []
    video_ious = []
    ...
    with open(os.path.join(out_dir, csv_file_name), 'w') as f:
        ...
        cf.writerow(video_names)
        cf.writerow(video_ious)
    logger.debug('Cityscapes Videos Mean IOU: %0.3f' % video_mean_iou)
    return video_mean_iou


def infer_on_cityscapes(model, data_loader, device, msc=False, flip=False, save_pred=False,
                        out_dir='./results/cityscapes/', msc_scales=None):
    compute_memory = True
    ...
    logger.debug('Cityscapes Videos Mean IOU: %0.3f' % video_mean_iou)
    print('Cityscapes Videos Mean IOU: %0.3f' % video_mean_iou)
    return video_mean_iou
...
def run_inference(args, device, model, load_state_dict=True, out_dir=None, videos=None):
    ...
    if args.dataset == 'davis':
        ...
        dataset_val = CityscapesValDataset(num_frames=args.num_frames, val_size=args.val_size,
                                          sequence_names=args.sequence_names, style=args.style,
                                          use_flow=args.use_flow)
    else:
    ...
    with torch.no_grad():
        if load_state_dict:
            # add weight_only beacuse (PyTorch ≥2.6) throws error unless weights_only=False is set.
            state_dict = torch.load(args.model_path, map_location=torch.device('cpu'), weights_only=False)['model']
            # change to the strict=False
        ...
        if args.dataset == 'moca':
            ...
            infer_on_bdd(model, data_loader_val, device, msc=args.msc, flip=args.flip,
                               save_pred=args.save_pred, out_dir=out_dir)
        elif args.dataset == 'cityscapes':
            infer_on_cityscapes(model, data_loader_val, device, msc=args.msc, flip=args.flip,
                               save_pred=args.save_pred, out_dir=out_dir)
        else:
