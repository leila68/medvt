...
def check_image_existence(kittimots_rgb_training_path, kittimots_gt_val_path):
    image_paths = []
    ...
    for folder_index, folder_name in enumerate(sorted(os.listdir(kittimots_rgb_training_path))):
        ...
        if os.path.isdir(folder_path):
            ...
            for image_index, image_name in enumerate(sorted(os.listdir(folder_path))):
                ...
                gt_path = os.path.join(kittimots_gt_val_path, folder_name)
                if os.path.exists(gt_path):
                    gt_path = os.path.join(kittimots_gt_val_path, folder_name, image_name)
...
def test_result(kittimots_rgb_training_path, kittimots_gt_val_path, dir_name):

    path1 = os.path.join(kittimots_rgb_training_path, dir_name)
    path2 = os.path.join(kittimots_gt_val_path, dir_name)
...
def create_gt_files(kittimots_val_json_file, kittimots_gt_val_path):
    coco_ds = COCO(kittimots_val_json_file)
    ...
    for img_id in img_ids:
        ...
        img_path = image_path(kittimots_gt_val_path, img_load['file_name'])
        save_mask_images(kittimots_gt_val_path, combined_mask, img_load['file_name'])
